...
func TestMLXBuildValidatesShapesBeforeCallingNativeCode(t *testing.T) {
	tests := []struct {
		...
		{name: "zero", shape: []int{2, 0}},
		{name: "negative", shape: []int{2, -1}},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, err := NewFloat32([]float32{1, 2}, tt.shape); err == nil {
...
func TestMLXBuildValidatesReshapeShape(t *testing.T) {
	tests := []struct {
		...
		{name: "empty", shape: nil, wantErr: true},
		{name: "zero", shape: []int{0, 2}, wantErr: true},
...
func TestMLXBuildValidatesNewAPIInputsBeforeCallingNativeCode(t *testing.T) {
	if _, err := Zeros(nil, Float32); err == nil {
		t.Fatal("expected Zeros with empty shape to fail")
	}
	if _, err := Ones([]int{1, 0}, Float32); err == nil {
	...
	if _, err := BroadcastTo(Array{}, nil); err == nil {
		t.Fatal("expected BroadcastTo with empty shape to fail")
	}
	if _, err := ExpandDimsAxes(Array{}, nil); err == nil {
		t.Fatal("expected ExpandDimsAxes with empty axes to fail")
	}
	if _, err := RandomNormal(nil, Float32, 0, 1); err == nil {
		t.Fatal("expected RandomNormal with empty shape to fail")
	}
	if _, err := RandomUniform([]int{2, 2}, DType(99), 0, 1); err == nil {
		t.Fatal("expected RandomUniform with invalid dtype to fail")
	}
	if _, err := RandomBernoulli(nil, 0.5); err == nil {
		t.Fatal("expected RandomBernoulli with empty shape to fail")
	}
}
