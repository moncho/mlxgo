...
The adapter can also replace selected session `wkv`, `wq_b` and grouped `wo_a` projections through
...
weights with checkpoint FP8 bytes, omit their float32 names from `parameters`:
...
model, err := deepseek.NewModelWithOptions(config, parameters, deepseek.ModelOptions{
    FP8AttentionKV: map[int]deepseek.FP8AttentionWeight{
        0: {Data: wkvBytes, Scales: kvBlockScaleBytes},
    },
    FP8AttentionQB: map[int]deepseek.FP8AttentionWeight{
        0: {Data: wqbBytes, Scales: qbBlockScaleBytes},
    },
    FP8AttentionOA: map[int]deepseek.FP8AttentionWeight{
        0: {Data: woaBytes, Scales: oaBlockScaleBytes},
    },
})
```

These options are independent. For layer 0 they replace `layers.0.attn.wkv.weight`
with logical shape `[HeadDim, Dim]`, and `layers.0.attn.wq_b.weight` with shape
`[Heads*HeadDim, QRank]`. Both dimensions must be divisible by 32. The separate
compressor indexer's `wq_b` is not replaced.

`FP8AttentionOA` replaces `layers.0.attn.wo_a.weight` with logical shape
`[Groups*ORank, Heads*HeadDim/Groups]`. Each group projects only its own heads;
`ORank` and the per-group input width must be divisible by 32. The reference
converts this matrix to BF16. Packed execution is accepted **only when every
decoded weight is already exactly BF16-representable**; rounding-changing values
are rejected, not silently approximated. The sampled real layer-0 matrix passes
this check. This is not general BF16 conversion support or BF16 activation parity.

Values are E4M3FN bytes with one E8M0 scale per 32x32 block. The constructor copies the bytes,
...
Only selected `wkv`/`wq_b`/`wo_a` weights change. Activations, normalization and
other projections remain float32. No float32 copy of the replaced weight is retained. This does not
...
[whole-attention measurements](FP8_OUTPUT_VALIDATION.md).
...
The building-block APIs reject non-float32 arrays. Packed `wkv`/`wq_b`/`wo_a` weights are
accepted only through explicit model options, not implicit dtype conversion.
