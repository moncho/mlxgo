...
released model.** All parameters must currently be float32. Engram, vision,
DSpark, production quantization and tokenizer integration remain unsupported.
...
| Engram | Token-ID remapping, n-gram hashes and conditional memory lookups; signed square-root gate and visual-token masking |
...
decode and reference logits. Engram and production quantization still need
separate implementations and tests before pretrained support can be claimed.
...
is disabled; the head returns all positions. Engram, vision and DSpark are off.
